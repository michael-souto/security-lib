...
export class FavoriteService {
  ...
  addFavorite(favorite: UserFavorite) {
    if (!this.existsFavorite(favorite.entityId)) {
      ...
      this.userFavoriteCrudApiService.create(favorite, this.authService.getUserId()).subscribe((response) => {
  ...
  removeFavorite(entityId: string) {
    this.userFavoriteCrudApiService.delete(entityId, this.authService.getUserId()).subscribe((response) => {
      this.myFavorites = this.myFavorites.filter((f) => f.entityId !== entityId);
      this.eventBus.emit({ type: 'favorite:updated' });
